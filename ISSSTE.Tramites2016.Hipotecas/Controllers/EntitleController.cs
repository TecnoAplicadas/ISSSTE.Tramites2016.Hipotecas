...
namespace ISSSTE.Tramites2016.Hipotecas.Controllers
{
	...
	public class EntitleController : BaseController
	{
		...
		public ActionResult Documentacion(string NoIssste, string RequestId, String Sts)
		{
			...
			try
			{
				...
				foreach (var document in result)
				{
					...
					else if (document.BeneficiarieType == 4 && tipoCred == "true")
					{
						documentType.Add(document);
						if (document.Required == true)
						{
			...
			try
			{
				...
				else
				{
					...
					if (contadorDoctos == Request.Files.Count)
					{
						...
						//int num = (int)Session["Status"];
						//TempData["currentStep"] = 2;
						//if (Session["Status"] != null)
						//{
						//    if ((int)Session["Status"] == (int)StatusEnum.EnesperaderevisiondedocumentacionDer)
						//    {
						//        requestBussinessLogic.SaveStatusNext(205, RequestId);
						//    }
						//}
						...
						// int num = Convert.ToInt32(Session["StsSend"].ToString());
						TempData["currentStep"] = 2;
		...
		public ActionResult PrivacidadIntegral()
		{
			return View();
		}
